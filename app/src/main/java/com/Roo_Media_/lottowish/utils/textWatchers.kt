...
class CurrencyTextWatcher(
    ...
    override fun onTextChanged(seq: CharSequence?, start: Int, before: Int, count: Int) {
        ...
        val seqLoad = CoroutineScope(Dispatchers.IO).async {
            when (before) {
                ...
                val selection = when (text.length) {
                    2 -> 2
                    else -> 1
                }.run {
...
class NumberTextWatcher(
    ...
    private val tmp = editTexts[position].setOnKeyListener { v, keyCode, event ->
